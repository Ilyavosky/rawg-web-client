...
export default function GenreFilter({ genres, selected, onSelect }: GenreFilterProps) {
  return (
    ...
    >
      <button
        onClick={() => onSelect("")}
        className={`font-sans text-[0.85rem] py-2 px-8 rounded-full border-2 border-rawg-brown transition-all duration-200 ${
      ...
      {genres.map((genre) => (
        <button
          ...
          className={`font-sans text-[0.85rem] py-[0.3rem] px-[0.8rem] rounded-full border-2 border-rawg-brown transition-all duration-200 ${
